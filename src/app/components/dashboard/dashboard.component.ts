...
export class DashboardComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.taskForm.valid) {
      ...
      this.http.post<any>(`${environment.BASE_URL}/createtask`, taskData, { headers }).subscribe(
        response => {
  ...
  getTasks(): void {
    ...
    this.http.get<any>(`${environment.BASE_URL}/gettask`, { headers }).subscribe(
      response => {
        console.log(response);
  ...
  deleteTask(taskId: string): void {
    ...
    this.http.delete<any>(`${environment.BASE_URL}/deletetask/${taskId}`, { headers }).subscribe(
      response => {
        console.log(response);
  ...
  markAsComplete(taskId: string): void {
    ...
    this.http.patch<any>(`${environment.BASE_URL}/edittask/${taskId}`, { status: true }, { headers }).subscribe(
      response => {
        console.log(response);
        alert(response.message)
  ...
  markAsImportant(taskId: string): void {
    ...
    this.http.patch<any>(`${environment.BASE_URL}/edittask/${taskId}`, { important: true }, { headers }).subscribe(
      response => {
        console.log(response);
        alert(response.message)
